...
static void pokey_act_uninitialized(void) {
    ...
    if (o->oDistanceToMario < 2000.0f * configDrawDistanceMultiplier || configDisableDrawDistance) {
...
static void pokey_act_wander(void) {
    ...
    if (o->oPokeyNumAliveBodyParts == 0) {
        obj_mark_for_deletion(o);
    } else if (o->oDistanceToMario > 2500.0f * configDrawDistanceMultiplier && !configDisableDrawDistance) {
